...
export const getCredits = cache(async (userId: string | null, isPro: boolean) => {
        ...
        if (rpcError) {
                ...
                if (error.code === "PGRST116") {
                    ...
                    return {
                        ...
                        resetTime: newCredits?.last_reset_timestamp
                            ? new Date(new Date(newCredits.last_reset_timestamp).getTime() + 24 * 60 * 60 * 1000).toISOString()
                            : null,
                    };
                }
            ...
            return {
                ...
                resetTime: data.last_reset_timestamp
                    ? new Date(new Date(data.last_reset_timestamp).getTime() + 24 * 60 * 60 * 1000).toISOString()
                    : null,
            };
        }
...
// Versión sin cache para cuando necesites datos frescos
export async function getCreditsUncached(userId: string | null, isPro: boolean) {
    if (isPro) {
        return { credits: -1, resetTime: null };
    }

    if (!userId) {
        return { credits: FREE_CREDITS_LIMIT, resetTime: null };
    }
